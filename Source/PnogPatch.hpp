...
public:
  ...
  void processAudio(AudioBuffer& audio) override
  {
    ...
    const float padLeftEnvTime = 
      vessl::math::interp<vessl::math::easing::expo::in>(PAD_ENV_MAX, PAD_ENV_MIN, pinPadLeftSpeed.getValue());
    const float padRightSpeed = PAD_SPEED_MIN + PAD_SPEED_MAX*pinPadRightSpeed.getValue();
    const float padRightEnvTime = 
      vessl::math::interp<vessl::math::easing::expo::in>(PAD_ENV_MAX, PAD_ENV_MIN, pinPadRightSpeed.getValue());
    ...
    for (count_t i = 0; i < size; i++)
    {
      ...
      const float sl = 1.0f - vessl::math::interp<vessl::math::easing::expo::out>(0.f, 1.f, inputLeft[i]*0.5f + 0.5f);
      const float sr = 1.0f - vessl::math::interp<vessl::math::easing::expo::out>(0.f, 1.f, inputRight[i]*0.5f + 0.5f);
